<div class="container-fluid">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form wire:submit.prevent="submit">
                <div class="mb-3">
                    <input type="text" wire:model="name" placeholder="name" class="form-control" id="name" />
                    @error('name')
                ...
                <div class="mb-3">
                    ...
                    <div class="d-flex gap-2">
                        <select wire:model.defer="dep_category_id" class="form-control" id="dep_category_id">
                            ...
                            <optgroup label="Main Categories">
                                @foreach ($depCategories->where('is_main', true) as $depcat)
                                    <option value="{{ $depcat->id }}"
                                        @if ($dep_category_id == $depcat->id) selected @endif>
                                        {{ $depcat->name }}
                                    </option>
                                @endforeach
                            </optgroup>
                        ...
                        <button type="button" class="btn btn-primary btn-xs text-nowrap" data-bs-toggle="modal"
                                    ...
                                    " />
                                        ...
                                        <div class="mt-2">
                                            <input type="radio" wire:model="featuredImageIndex"
                                                ...
                                                class="form-check-input @error('featuredImageIndex') is-invalid @enderror"
                                                id="existing_featured_{{ $eIndex }}" name="featured" required />
                                            <label class="form-check-label d-block small"
                                                for="existing_featured_{{ $eIndex }}">
                                                Featured Image
                                            </label>
                                        </div>
                                        @if ($featuredImageIndex == 'existing_' . $eIndex || (isset($image['is_featured']) && $image['is_featured']))
    ...
    <!-- Category Modal -->
    <div class="modal fade" id="ManageDepCategoryModal" tabindex="-1" aria-labelledby="ManageDepCategoryModalLabel"
        aria-hidden="true">
                ...
                .then(editor => {
                    ...
                    editor.model.document.on("change:data", () => {
                        ...
                        debounceTimer = setTimeout(() => {
                            ...
                            const currentCategoryId = document.getElementById('dep_category_id')
                                .value;
