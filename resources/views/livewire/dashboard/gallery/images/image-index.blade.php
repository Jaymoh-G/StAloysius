            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row mb-4 align-items-center">
                        ...
                        <div class="col-md-3">
                            <input type="text" class="form-control" placeholder="Search images..." wire:model.live="searchTerm">
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            <div class="card">
                                    ...
                                    <ul class="nav nav-tabs card-header-tabs mt-2">
                                        <li class="nav-item">
                                            <a class="nav-link {{ $activeTab === 'upload' ? 'active' : '' }}"
                                               href="#"
                                               wire:click.prevent="setActiveTab('upload')">
                                        ...
                                        <li class="nav-item">
                                            <a class="nav-link {{ $activeTab === 'existing' ? 'active' : '' }}"
                                               href="#"
                                               wire:click.prevent="setActiveTab('existing')">
                                ...
                                <div class="card-body">
                                    ...
                                    @if ($alertMessage)
                                        <div class="alert alert-{{ $alertType === 'error' ? 'danger' : $alertType }} alert-dismissible fade show">
                                            {{ $alertMessage }}
                                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close" wire:click="$set('alertMessage', '')"></button>
                                        </div>
                                    @endif

                                    <div id="alertContainer"></div>

                                    <form wire:submit.prevent="{{ $isEditing ? 'updateImage' : ($activeTab === 'upload' ? 'uploadImages' : 'addExistingImagesToAlbum') }}" id="imageForm">
                                        <div class="mb-3">
                                        ...
                                        <div class="mb-3">
                                            <div class="d-flex justify-content-between align-items-center mb-2">
                                                ...
                                                <div>
                                                    <button type="button" class="btn btn-sm btn-outline-secondary me-2"
                                                        ...
                                                        <i class="fa fa-filter"></i> {{ $showOnlyUnused ? 'Show All' : 'Show Unused' }}
                                                    </button>
                                                    <button type="button" class="btn btn-sm {{ $showExistingImageActions ? 'btn-danger' : 'btn-outline-danger' }}"
                                                            wire:click="toggleExistingImageActions">
                                                        <i class="fa fa-trash"></i> {{ $showExistingImageActions ? 'Cancel Delete' : 'Delete Mode' }}
                                                    </button>
                                                </div>
                                            </div>
                                            ...
                                            @if($showExistingImageActions)
                                                <div class="alert alert-warning">
                                                    <i class="fa fa-exclamation-triangle"></i> Delete mode active. Select images to permanently delete from storage.
                                                    <div class="mt-2">
                                                        ...
                                                        <button type="button" class="btn btn-outline-secondary btn-sm" wire:click="toggleExistingImageActions">
                                                            Cancel
                                                        </button>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="existing-images-container mt-2" style="max-height: 500px; overflow-y: auto;">
                                                <div class="row g-2">
                                                                ...
                                                                @if(!$image['isUsed'])
                                                                    <div class="position-absolute top-0 start-0 p-1">
                                                                        <span class="badge bg-warning">
                                                                            <i class="fa fa-exclamation-triangle"></i> Unused
                                                                        </span>
                                                                    </div>
                                                                @endif
                                                            </div>
                                                    ...
                                                    @empty
                                                        <div class="col-12">
                                                            <div class="alert alert-info">
                                                                No {{ $showOnlyUnused ? 'unused ' : '' }}existing images found.
                                                            </div>
                                                        </div>
                                                    @endforelse
                                                </div>
                                            ...
                                            @if(!$showExistingImageActions)
                                                <button type="submit" class="btn btn-primary me-2" id="submitBtn">
                                                    ...
                                                    @elseif($activeTab === 'upload')
                                                        Upload Images{{ $album_id ? ' to ' . $albums->firstWhere('id', $album_id)->title : '' }}
                                                    @else
                                                        Add Images{{ $album_id ? ' to ' . $albums->firstWhere('id', $album_id)->title : '' }}
                                                    @endif
                                                </button>
                                                @if($isEditing)
                                ...
                                @forelse($galleryImages as $image)
                                    <div class="col">
                                        <div class="card h-100">
                                            ...
                                            <div class="card-footer d-flex justify-content-end py-1 px-2">
                                                <button class="btn btn-xs btn-info me-1" wire:click="editImage({{ $image->id }})">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (imageForm && submitBtn) {
            ...
            submitBtn.addEventListener('click', function(e) {
                // Get the active tab
                const isUploadTab = imageForm.getAttribute('wire:submit.prevent').includes('uploadImages');
                const isExistingTab = imageForm.getAttribute('wire:submit.prevent').includes('addExistingImagesToAlbum');
